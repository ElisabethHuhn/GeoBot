...
/**
 * Created by elisabethhuhn on 4/3/2016.
 * The conversions that the class performs are accurate to within 1 meter.
 */
class IBMCoordinateConversion {


    IBMCoordinateConversion()
    {

    }

    /**
     * This method does actual conversion from UTM to Lat/Long
     * @param UTM coordinates to be converted.
     *            Format of these coordinates is ????
     *            UTM string (which is in the format 34 G 683473 4942631)
    ...
    /**
     * perform conversion from latitude/longitude to UTM coordinates
     * NOTE that integer coordinates are returned!
     *
     * If either latitude or longitude is invalid
     * (i.e. not within valid coordinate range)
     * an IllegalArgumentException is thrown
     * @param latitude value
     * @param longitude value
     * @return UTM coordinate
     * returns UTM coordinates as a 15-character string
     * (that is, to 1-meter precision)
     * UTM String format:
     *  longZone + " " + latZone + " " + ((int) _easting) + " " + ((int) _northing);
     *
     */
    ...
    String latLon2PreciseUTM(double latitude, double longitude)
    {
        LatLon2UTM c = new LatLon2UTM();
        return c.convertLatLonToPreciseUTM(latitude, longitude);

    }

    /**
     * Assure that the input coordinates are valid.
     * If not, an exception is thrown. Caller needs to catch this exception
     * @param latitude  value
     * @param longitude value
     */
    ...
    /**
     *      convert lat/long coordinates to MGR UTM format
     * @param latitude  value
     * @param longitude value
     * @return MGR UTM format String
     */
    String latLon2MGRUTM(double latitude, double longitude)
    {
    ...
    /**
     * Convert MGRUTM coordinates to lat/long format
     * @param MGRUTM format of coordinates is ?????
     * @return Lat/Long coordinates
     */
    double[] mgrutm2LatLon(String MGRUTM)
    {
    ...
    /**
     * convert degrees of lat/long to Radians
     *   and vice verse
     * @param degree
    ...
    /**
     * Wrappers for some useful math routines
     * @param a value
     * @param b value
    ...
    private class LatLon2UTM
    {
        /**
         * perform conversion from latitude/longitude to UTM coordinates
         * NOTE that coordinates to integer precision are returned!
         *
         * If either latitude or longitude is invalid
         * (i.e. not within valid coordinate range)
         * an IllegalArgumentException is thrown
        ...
        String convertLatLonToPreciseUTM(double latitude, double longitude)
        {
            ...
            String stringEasting = String.valueOf(_easting);
            String stringNorthing = String.valueOf(_northing);

            //UTM = longZone + " " + latZone + " " + ((int) _easting) + " "
            //        + ((int) _northing);

             UTM = longZone + " " + latZone + " " + stringEasting + " " + stringNorthing;
        ...
        /**
         * sets various variables required for calculating conversions
         * @param latitude  value
         * @param longitude value
         */
        void setVariables(double latitude, double longitude)
        {
            ...
            A6 = (POW(p * sin1, 6.) * nu * SIN(latitude) * POW(COS(latitude), 5.) / 720.)
                    * (6.1 - 58. * POW(TAN(latitude), 2.) + POW(TAN(latitude), 4.) + 270.
         ...
         * @param latitude value
         * @return Northing & Easting values
         */
        ...
        // equatorial radius
        //WGS84 value as referenced by Karney
    ...
    {
        String convertLatLonToMGRUTM(double latitude, double longitude)
        {
            validate(latitude, longitude);
            String mgrUTM = "";
    ...
    /**
     * conversion from MGRS is incorrect!!! Do not use!
     */
    private class MGRUTM2LatLon extends UTM2LatLon
    {
        double[] convertMGRUTMToLatLong(String mgrutm)
        {
    ...
    private class UTM2LatLon
    {
        ...
        /**
         * determine the hemisphere that the location denoted by the string is in.
         * @param latZone value
        ...
        double[] convertUTMToLatLong(String UTM)
        {
            double[] latlon = { 0.0, 0.0 };
        ...
        /**
         * sets the variables required for calculation and then
         * immediately calculates latitude.
         * Longitude is calculated using the longitude zone.
         */
        void setVariables()
        {
            arc = northing / k0;
    ...
    private class Digraphs
    {
        ...
        String getDigraph1(int longZone, double easting)
        {
            int a1 = longZone;
        ...
        String getDigraph2(int longZone, double northing)
        {
            int a1 = longZone;
            double a2 = 1. + 5. * ((a1 - 1.) % 2.);
            double a3 = northing;
            double a4 ; // originally = (a2 + ((int) (a3 / 100000.)));
